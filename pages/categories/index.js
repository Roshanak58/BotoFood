const { default: CategoriesPage } = require("@/components/templates/CategoriesPage");
...
export async function getServerSideProps(context) {
  ...
  const filteredData=data.filter((item)=>{
    ...
    if (!time&& difficulty&& difficultyResult.length){
      return item
    }
    if (time&& !difficulty&& timeResult.length ){
      return item
    }
  })
